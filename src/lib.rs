...
pub fn find_error_nums_645(nums: Vec<i32>) -> Vec<i32> {
    ...
    let dup = nums
        .into_iter()
        .filter(|n| set.insert(*n).not())
        .collect::<Vec<_>>();

    let mis = (1..set.last().unwrap().to_owned() + 2).find(|num| set.insert(*num));

    vec![dup.first().unwrap().to_owned(), mis.unwrap()]
}

pub fn pseudo_palindromic_paths_1457(root: Option<Rc<RefCell<TreeNode>>>) -> i32 {
    fn recurse(node: Option<Rc<RefCell<TreeNode>>>, mut nodes: u16) -> i32 {
        ...
        if right.is_none() && left.is_none() {
            return if is_pseudopalindrome(nodes) { 1 } else { 0 };
        }

        return recurse(left, nodes.clone()) + recurse(right, nodes);
    }

    #[inline]
    fn is_pseudopalindrome(nodes: u16) -> bool {
        let odds = nodes.count_ones();
...
// pub fn longest_common_subsequence_1143(text: (String, String)) -> i32 {
...
//         // let binding = text1.clone();
//         // let mut sh_text_1 = binding.as_str();
//         let binding = text2.clone();
//         let mut sh_text_2 = &binding.chars().enumerate().collect::<Vec<_>>()[..];

//         println!("sh: {sh_text_2:?}");

//         let mut usable_1 = BTreeMap::new();
//         for c in text1.chars() {
//             usable_1.entry(c).and_modify(|it| *it += 1).or_insert(1);
//         }

//         let mut usable_2 = BTreeMap::new();
//         for c in text2.chars() {
//             usable_2.entry(c).and_modify(|it| *it += 1).or_insert(1);
//         }

//         let text_1_set = usable_1
//             .keys()
//             .into_iter()
//             .cloned()
//             .collect::<BTreeSet<_>>();
//         let text_2_set = usable_2
//             .keys()
//             .into_iter()
//             .cloned()
//             .collect::<BTreeSet<_>>();

//         // let text_1_set = text1.chars().collect::<BTreeSet<_>>();
//         // let text_2_set = text2.chars().collect();
//         let common_chars = text_1_set
//             .intersection(&text_2_set)
//             .into_iter()
//             .collect::<String>();

//         println!("{text1} {text2} common: {common_chars}");

//         let positions_1 = text1
//             .chars()
//             .map(|c| (c, common_chars.find(c)))
//             .map(|(c, pos1)| match pos1 {
//                 Some(_) => char::from_digit(text2.find(c).unwrap() as u32, 10).unwrap(),
//                 None => ' ',
//             })
//             .collect::<String>();
...
//         // let positions_2 = text2
//         //     .chars()
//         //     .map(|c| (c, common_chars.find(c)))
//         //     .map(|(c, pos2)| match pos2 {
//         //         Some(_) => char::from_digit(text1.find(c).unwrap() as u32, 10).unwrap(),
//         //         None => ' ',
//         //     })
//         //     .collect::<String>();
//         // // println!("{positions_1} {positions_2}");

//         println!(
//             "v > 1: {:?}",
//             usable_1.iter().filter(|(k, v)| v > &&1).collect::<Vec<_>>()
//         );

//         let positions_1 = text1
//             .chars()
//             .filter_map(|c| common_chars.find(c).map(|_| c))
//             .filter_map(|c| {
//                 let Some((pos, &(found, _))) = sh_text_2
//                     .iter()
//                     .enumerate()
//                     .find(|(pos, &(found, char))| char == c)
//                 else {
//                     return None;
//                 };

//                 usable_1.entry(c).and_modify(|counter| *counter -= 1);
//                 if usable_1[&c] < 0 {
//                     println!("none: {c}: {}", usable_1[&c]);
//                     return None;
//                 } else {
//                     println!("some: {c}: {}", usable_1[&c]);
//                     println!("found: {}", found);
//                     sh_text_2 = &sh_text_2[pos + 1..];
//                     println!("sh: {sh_text_2:?}");
//                     return Some(found);
//                 }
//             })
...
//         //     .filter_map(|c| common_chars.find(c).map(|_| c))
//         //     .filter_map(|c| {
//         //         let Some(found) = text1.find(c) else {
//         //             return None;
//         //         };

//         //         usable_2.entry(c).and_modify(|counter| *counter -= 1);
//         //         if usable_2[&c] < 0 {
//         //             return None;
//         //         } else {
//         //             return Some(found);
...
//         let mut increasing_1 = vec![];
//         for (pos, &item) in positions_1.iter().enumerate() {
//             let mut item = item;
//             let i = pos;
//             let mut local_increasing = 0;

//             for &cmp_me in positions_1.iter().skip(i + 1) {
//                 println!("{cmp_me} & {item}");
//                 if cmp_me > item {
//                     println!("push");
//                     local_increasing += 1;
...
//             println!("local_increasing: {local_increasing}");
//             increasing_1.push(local_increasing);
//         }
//         println!("1: {increasing_1:?}");

//         // let mut increasing_2 = vec![];
//         // for (pos, &item) in positions_2.iter().enumerate() {
//         //     let mut item = item;
//         //     let i = pos;
//         //     let mut local_increasing = 0;

//         //     for &cmp_me in positions_2.iter().skip(i + 1) {
//         //         if cmp_me > item {
//         //             local_increasing += 1;
//         //             item = cmp_me;
//         //         }
...
//         //     increasing_2.push(local_increasing);
//         // }

//         // // println!("2: {increasing_2:?}");

//         let result_1 = increasing_1.into_iter().max().unwrap_or(-1) + 1;
//         // let result_2 = increasing_2.into_iter().max().unwrap_or(-1) + 1;

//         // for c in text1.chars() {
//         //     let position = sh_text_2.find(c);

//         //     sh_text_2 = position.map_or(sh_text_2, |pos| {
//         // // //         println!("{pos} {c} {result_1} {sh_text_2}");
//         //         result_1 += 1;
//         //         &sh_text_2[pos + 1..]
//         //     });
//         // }
...
//         // for c in text2.chars() {
//         //     let position = sh_text_1.find(c);

//         //     sh_text_1 = position.map_or(sh_text_1, |pos| {
//         // // //         println!("{pos} {c} {result_2} {sh_text_1}");
//         //         result_2 += 1;
//         //         &sh_text_1[pos + 1..]
//         //     });
//         // }

//         // // // println!("{text1} -> {sh_text_1} w/ result: {result_1}");
//         // // // println!("{text2} -> {sh_text_2} w/ result: {result_2}");

//         // result_1.max(result_2) as i32
...
pub fn is_valid_20(s: String) -> bool {
    ...
    for c in s.chars() {
        match c {
            opening @ '(' | opening @ '{' | opening @ '[' => stack.push(opening),
            ')' => {
                let Some(bracket) = stack.pop() else {
                    return false;
                };
                if bracket != '(' {
                    return false;
            ...
            unknown => unreachable!("illegal character {unknown} encoutnered"),
        }
    }
...
mod tests {
    ...
    test_1457_Pseudo_Palindromic_Paths_in_a_Binary_Tree_1,   pseudo_palindromic_paths_1457, cargo_leet::TreeRoot::from("[2,3,1,3,1,null,1]").into(),  2;
    test_1457_Pseudo_Palindromic_Paths_in_a_Binary_Tree_2,   pseudo_palindromic_paths_1457, cargo_leet::TreeRoot::from("[2,1,1,1,3,null,null,null,null,null,1]").into(),  1;
    test_1457_Pseudo_Palindromic_Paths_in_a_Binary_Tree_3,   pseudo_palindromic_paths_1457, cargo_leet::TreeRoot::from("[1,9,1,null,1,null,1,null,null,7,null,null,4]").into(),  1;
    // test_1457_Pseudo_Palindromic_Paths_in_a_Binary_Tree_4,   pseudo_palindromic_paths_1457, cargo_leet::TreeRoot::from(include_str!("1457_1").trim()).into(),  1;


    // test_1143_Longest_Common_Subsequence_1, longest_common_subsequence_1143, ("abcde".to_string(), "ace".to_string()), 3;
    // test_1143_Longest_Common_Subsequence_2, longest_common_subsequence_1143, ("abc".to_string(), "def".to_string()), 0;
    // test_1143_Longest_Common_Subsequence_3, longest_common_subsequence_1143, ("ezupkr".to_string(), "ubmrapg".to_string()), 2;
    // test_1143_Longest_Common_Subsequence_4, longest_common_subsequence_1143, ("oxcpqrsvwf".to_string(), "shmtulqrypy".to_string()), 2;
    // test_1143_Longest_Common_Subsequence_5, longest_common_subsequence_1143, ("hofubmnylkra".to_string(), "pqhgxgdofcvmr".to_string()), 5;
    // test_1143_Longest_Common_Subsequence_6, longest_common_subsequence_1143, ("abcba".to_string(), "abcbcba".to_string()), 5;

    test_20_Valid_Parentheses_1, is_valid_20, "()".to_string(), true;
    test_20_Valid_Parentheses_2, is_valid_20, "()[]{}".to_string(), true;
    test_20_Valid_Parentheses_3, is_valid_20, "(]".to_string(), false;
